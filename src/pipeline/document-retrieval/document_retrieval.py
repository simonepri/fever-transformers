...
class Doc_Retrieval:
    def __init__(self, database_path, add_claim=False, max_results=None):
            ...
            while i < 12:
                try:
                    ...
                    if self.max_results is not None:
                        predicted_pages.extend(docs[: self.max_results])
                    else:
    ...
    def exact_match(self, line):
        ...
        claim = claim.replace(".", "")
        claim = claim.replace("-", " ")
        words = [self.proter_stemm.stem(word.lower())
                 for word in self.tokenizer(claim)]
        words = set(words)
...
def main(db_file, max_results, in_file, out_file, add_claim=True, parallel=True):
    method = Doc_Retrieval(
        database_path=db_file, add_claim=add_claim, max_results=max_results
    )
    processed = dict()
    path = os.getcwd()
    jlr = JSONLineReader()
        ...
        with ThreadPool(processes=4 if parallel else None) as p:
            for line in tqdm(
                get_map_function(parallel, p)(
                    lambda l: process_line_with_progress(
                        method, l, progress), lines
                ),
                total=len(lines),
            ):
